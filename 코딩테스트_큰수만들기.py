...
def solution(number, k):
    ...
    while nl:         # nl리스트 내용(길이)만큼 반복할 것
        ...
        idx += 1                             # 주의점은 첫번째 이후 argmax 인덱스 값은 범위 지정을 위해 새로 만든 리스트의 인덱스 값으로 계산되기때문에
        count -= 1                           # 앞전의 인덱스를 더해야 원래 리스트에서의 인덱스값으로 쓸 수 있음 
        if count==0:                         # 다음번 비교할 범위리스트는 argmax로 나온 인덱스 다음꺼부터 해야되니까 인덱스+1 해줌
