...
def solution(number, k):
    ...
    for i in range(len(nl)): # 입력받은 숫자길이만큼 반복할 것
        ...
        new_list.append(nl[idx])             # 지정한 범위에서 제일 큰 곳의 인덱스 값을 반환해주고 해당 자리의 숫자가 뭔지도 저장함
        count -= 1                           # 주의점은 다음의 argmax 인덱스 값은 범위 지정을 위해 새로 만든 리스트의 인덱스 값으로 계산되기때문에
        idx += 1                             # 앞전의 인덱스를 더해야 원래 리스트에서의 인덱스값으로 쓸 수 있음 
        if count==0:                         # 수를 하나 뽑았으니 남은 뽑을 개수를 하나 차감함, 이때 뽑을 개수가 0이면 다뽑은거니까 반복 종료
            break                            # 다음번 비교할 범위리스트는 argmax로 나온 인덱스 다음꺼부터 해야되니까 인덱스+1 해줌
