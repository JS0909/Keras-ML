...
# train_test_split을 위한 x1, x2, y1, y2 행값 맞춰주기
randidx = np.random.randint(y1_train.shape[0], size=x1_train.shape[0]-y1_train.shape[0])
y1_train_aug = y1_train[randidx]
randidx = np.random.randint(y2_train.shape[0], size=x2_train.shape[0]-y2_train.shape[0])
...
# 따로따로 나눠줌
x1_train, x1_test, y1_train, y1_test = train_test_split(x1_train, y1_train, train_size=0.8, shuffle=True, random_state=9)
x2_train, x2_test, y2_train, y2_test = train_test_split(x2_train, y2_train, train_size=0.8, shuffle=True, random_state=9)
...
print(x1_train.shape, x2_train.shape, y1_train.shape, y2_train.shape) # (2400, 150, 150, 3) (760, 150, 150, 3) (2400, 30) (760, 4)
print(x1_test.shape, x2_test.shape, y1_test.shape, y2_test.shape) # (600, 150, 150, 3) (191, 150, 150, 3) (600, 30) (191, 4)

np.save('d:/study_data/_save/_npy/_project/train_x1_.npy', arr =x1_train)
np.save('d:/study_data/_save/_npy/_project/train_x2_.npy', arr =x2_train)
np.save('d:/study_data/_save/_npy/_project/train_y1_.npy', arr =y1_train)
np.save('d:/study_data/_save/_npy/_project/train_y2_.npy', arr =y2_train)

np.save('d:/study_data/_save/_npy/_project/test_x1_.npy', arr =x1_test)
np.save('d:/study_data/_save/_npy/_project/test_x2_.npy', arr =x2_test)
np.save('d:/study_data/_save/_npy/_project/test_y1_.npy', arr =y1_test)
np.save('d:/study_data/_save/_npy/_project/test_y2_.npy', arr =y2_test)
print('데이터 수치화 저장 완료')
