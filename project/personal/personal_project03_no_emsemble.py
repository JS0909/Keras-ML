...
tf.random.set_seed(99) # 하이퍼 파라미터 튜닝 용이하게 하기 위해
...
x1_train = np.load(filepath+'train_x1_'+suffix)
x2_train = np.load(filepath+'train_x2_'+suffix)
y1_train = np.load(filepath+'train_y1_'+suffix)
y2_train = np.load(filepath+'train_y2_'+suffix)

x1_test = np.load(filepath+'test_x1_'+suffix)
x2_test = np.load(filepath+'test_x2_'+suffix)
y1_test = np.load(filepath+'test_y1_'+suffix)
y2_test = np.load(filepath+'test_y2_'+suffix)
...
output1 = Dense(30, activation='softmax')(drop1)

# 2-2. 나이 예측용 모델
# VGGNet 16 모델 구성 참고
input2 = Input(shape=(150, 150, 3))
conv1 = Conv2D(64,(3,3), padding='same', activation='relu')(input2)
conv2 = Conv2D(64,(3,3), activation='relu')(conv1)
mp1 = MaxPool2D(pool_size=(2,2))(conv2)
drop1 = Dropout(0.2)(mp1)
...
flat1 = Flatten()(mp4)
drop1 = Dropout(0.3)(flat1)
output2 = Dense(4, activation='softmax')(drop1)

model_breed = Model(inputs=input1, outputs=output1)
model_age = Model(inputs=input2, outputs= output2)
# model.summary()
...
log1 = model_breed.fit(x1_train, y1_train, epochs=500, batch_size=32, callbacks=[Es], validation_split=0.2)
log2 = model_age.fit(x2_train, y2_train, epochs=500, batch_size=32, callbacks=[Es], validation_split=0.2)

model_breed.save('D:/study_data/_save/_h5/project_breed.h5')
model_age.save('D:/study_data/_save/_h5/project_age.h5')

# model_breed = load_model('D:/study_data/_save/_h5/project_breed.h5')
# model_age = load_model('D:/study_data/_save/_h5/project_age.h5')


#4. 평가, 예측
loss_breed = model_breed.evaluate(x1_test, y1_test)
loss_age = model_age.evaluate(x2_test, y2_test)
print('tested loss : ', loss_breed)
print('tested loss : ', loss_age)

y1_pred= model_breed.predict(x1_test)
y2_pred = model_age.predict(x2_test)
...
# 테스트용 이미지로 프레딕트
testing_img = np.load(filepath+'testing_img_'+suffix)

testpred_breed = model_breed.predict(testing_img)
testpred_age = model_age.predict(testing_img)
